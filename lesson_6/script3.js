...
const phon = document.querySelector('.phones');

let phoneNum = [];
for(let i = 0; i < users.length; i++) {
    if (users[i].balance > 2000) {
        phoneNum.push(users[i].phone);
    }
}
phon.innerHTML = `${phoneNum.join('<br>')}`;

let sumBalance = 0;
for(let i = 0; i < users.length; i++) {
    sumBalance += users[i].balance;
}
bal.innerHTML = `Sum of all accounts - ${sumBalance.toFixed(2)}`;